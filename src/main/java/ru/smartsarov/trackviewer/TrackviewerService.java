...
public class TrackviewerService {
	...
	public Response getVehicleTypes() {
		...
		try {
			return Response.status(Response.Status.OK).entity(new Gson().toJson(Trackviewer.getVehicleTypes())).build();
		} catch (ClassNotFoundException | SQLException e) {
			// TODO Auto-generated catch block
			return Response.status(Response.Status.OK).entity(e.toString()).build();
		}
	...
	@Path("/track/get_json")
	@Produces(MediaType.APPLICATION_JSON + ";charset=UTF-8")
	public Response getTrackJson(@QueryParam("min_ts") long min_ts, @QueryParam("max_ts") long max_ts,
			@QueryParam("vehicle_number") String vehicleNumber) {
		try {
			return Response.status(Response.Status.OK).entity(Trackviewer.jsonTrackData(min_ts, max_ts,
					vehicleNumber == null ? null : vehicleNumber.toLowerCase())).build();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			return Response.status(Response.Status.OK).entity(e.toString()).build();
		}
	...
	@Path("/track/timeline")
	@Produces(MediaType.APPLICATION_JSON + ";charset=UTF-8")
	public Response getJsonDay(@QueryParam("ts") long ts, @QueryParam("type") String type) {
		try {
	...
	public Response getGeoLocationList(String jsonRequest) {
		try {
			...
			return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
					.entity(Trackviewer.getJsonMessage(e.toString())).build();
		}
	}
	...
	@Produces(MediaType.APPLICATION_JSON + ";charset=UTF-8")
	public Response getJsonWeek(@QueryParam("ts") long ts, @QueryParam("type") String type) {
		try {
	...
	@Produces(MediaType.APPLICATION_JSON + ";charset=UTF-8")
	public Response generateHourReport(@QueryParam("ts") long ts) {
		try {
	...
	@Produces(MediaType.APPLICATION_JSON + ";charset=UTF-8")
	public Response generateHourlyForDay(@QueryParam("ts") long ts, @QueryParam("type") String type) {
		try {
	...
	public StreamingOutput getCommonPdf(@QueryParam("ts_min") long ts_min, @QueryParam("ts_max") long ts_max,
	...
	@Produces({ "application/pdf" })
	public StreamingOutput getCommonPdfByNumberList(@QueryParam("ts_min") long ts_min,
			@QueryParam("ts_max") long ts_max, @QueryParam("numList") String numList) {
		try {
			return new StreamingOutput() {
				@Override
				public void write(OutputStream output) throws IOException, WebApplicationException {
	...
	@Produces({ "application/pdf" })
	public StreamingOutput getSpecificPdf(@QueryParam("ts_min") long ts_min, @QueryParam("ts_max") long ts_max,
		...
		return new StreamingOutput() {
			...
			public void write(OutputStream output) throws IOException, WebApplicationException {
				...
				} catch (DocumentException | ClassNotFoundException | SQLException | TrackviewerException e) {
